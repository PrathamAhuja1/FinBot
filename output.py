...
def generate_final_answer(query, index_name):
    if not generator:
        print("Assistant: I'm experiencing technical difficulties. Please try again later.")
        return {
    ...
    try:
        ...
        # Get context and API data
        internal_context = get_internal_context(query, index_name)
        api_responses = determine_api_calls(query)
        
        # Print clean data sources
        print("\n[Internal Knowledge Base]")
        print("─" * 50)
        print(internal_context[:1000] + ("..." if len(internal_context) > 1000 else ""))
        print("─" * 50)
        
        # Print formatted API data
        print("\n[Market Data]")
        print("═" * 50)
        for source, data in api_responses.items():
            if source.startswith("_") or isinstance(data, str):
                continue
                
            if source == "stock_data" and "price" in data:
            ...
            elif source == "crypto" and "price" in data:
                print(f"💰 {data.get('name', 'Crypto')} ({data.get('symbol', '')}):")
                print(f"   Price: ${data.get('price', 0):,.2f}")
                print(f"   Market Cap: ${data.get('marketCap', 0):,.0f}")
                print(f"   24h Volume: ${data.get('24hVolume', 0):,.0f}\n")
                
            elif source == "forex" and "rate" in data:
                print(f"🌍 {data.get('base', '')}/{data.get('target', '')}:")
                print(f"   Exchange Rate: {data.get('rate', 0):.4f}")
                print(f"   Last Updated: {data.get('timestamp', 'N/A')}\n")
                
            elif source == "metal_prices":
                print(f"🪙 {data.get('metal', 'Metal')}:")
                print(f"   Price: {data.get('rate', 0):.2f} {data.get('currency', '')}/gram")
                print(f"   Last Updated: {data.get('timestamp', 'N/A')}\n")
        
        print("═" * 50)
        
        # Generate and print final answer
        ...
        clean_answer = full_response.split("<|start_header_id|>assistant<|end_header_id|>")[-1].split("<|")[0].strip()
        ...
        print(f"\n[System Error]: I encountered an issue processing your request. Please try again.")
        return {
...
if __name__ == "__main__":
    while True:
        ...
        if query.lower() in ['exit', 'quit', 'no']:
            print("\n🛑 Exiting financial assistant...")
            break

        generate_final_answer(query, INDEX_NAME)
